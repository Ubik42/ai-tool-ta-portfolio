...
- Presenter Pack：探测 38 个关键证据文件是否存在，并把总体 gate 压成 `CapturePending` / `Review` / `Ready`。
...
- `Task Orchestrator` 证据视图顶部的 `R40 Cross-DCC / Engine Reviewer Pack`
- `Export Case Page` 按钮
- `Presenter Pack` 按钮
- `Preset Facts` 按钮
- `Txn Guard` 按钮
...
- Maya 2024 `mayapy` presenter pack smoke 通过：
  ...
  - package：`ai-tool-ta-dcc-first-showcase-r40` / `dcc-first-package@1.37.0`
  - gate：`CapturePending`
  - demo route steps：29
  - key evidence files：38
  - present/missing required evidence files：38 / 0
