...
更新时间：2026-08-05  
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r35` / `dcc-first-package@1.32.0`
...
- Presenter Pack：`<repo>\dcc-hosts\maya-auroraview-host\artifacts\r35-character-calibration-drilldown-presentation-pack-20260805-202448.json`
...
- Presenter Pack 结果：32 / 32 evidence files present，0 missing required files，24 demo route steps。
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R35 Presenter Pack 32/32 evidence present；24 步 demo route；新增 Character Calibration Drilldown probe |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 95% | Maya 内打开工具、外部 command bridge、24 步 demo route、32 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
3. 每个 Maya 会话启动外部控制 bridge，然后外部 shell 控制：
   ...
   python <repo>\dcc-hosts\maya-auroraview-host\scripts\send_maya_command.py --export-presenter-pack r35-character-calibration-drilldown-presentation-pack
   ```
...
下一轮不要再围绕 Blender/Max readiness 或 Unreal missing fixture 打转，它们已进入真实 runtime 证据。`Character Calibration Studio` 已有 Maya L3 和 R35 drilldown，`Spatial Authoring Workbench` 已从计划进入 Maya L3，`Platform Variant Forge` 已完成 L3-linked plan、Unreal runtime-vs-plan L3、dry-run generation plan、texture runtime collector、public Texture2D payload fixture、controlled executor 和 executor receipts；后续优先深化 Spatial Authoring 的 Maya UI drilldown、Character Calibration 的 Unreal Control Rig 对照，或把 R34 receipts 转成更细的 StaticMesh LOD/Nanite public runtime post-check。
