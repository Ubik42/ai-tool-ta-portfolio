...
更新时间：2026-08-06
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r68` / `dcc-first-package@1.65.0`
...
- 3ds Max L3：`<repo>\dcc-hosts\3dsmax-rule-adapter\artifacts\max-rule-adapter-l3-20260806-032411.json`
- Presenter Pack 结果：65 / 65 evidence files present，0 missing required files，55 demo route steps。
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R68 Presenter Pack 66/66 evidence present；56 步 demo route；新增 Unreal Animation Notify Native Bridge Readiness |
...
| 14 | Unreal Animation Bridge / Deep Facts / Attach Timing / Native Notify Bridge | 把 Maya 动画连续性 facts 映射到 Unreal AnimSequence/Skeleton/root motion/curve/compression runtime facts，并继续判断 gameplay attach timing 是否可发布 | Unreal import L3 完成；R41 deep facts 完成；R67 attach timing readiness 完成；R68 native notify bridge source/readiness 完成，missingRequiredNativeFiles=0，binary/commandlet 待 build |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、56 步 demo route、66 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Unreal Animation Bridge | 74% | Maya 生成 FBX、Unreal Python 导入 Skeleton/SkeletalMesh/AnimSequence、2/2 sequences present；R41 只读采集 duration、derived frame span、frame-rate、root motion、compression metadata visibility；R67 把 gameplay attach intent 接到 AnimSequence notify/timing gate；R68 已新增 `AI_Tool_TA_AnimNotifyBridge` source/readiness，runtimeEntered=true，missingRequiredNativeFiles=0，productionWrites=0 | native bridge binary / commandlet diagnostics 未 build；curve names / authored notify 写入仍待后续 |
