...
更新时间：2026-08-05  
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r50` / `dcc-first-package@1.47.0`
...
- Presenter Pack 结果：49 / 49 evidence files present，0 missing required files，39 demo route steps。
...
| 19 | Groom Export Inspector / Unreal Readiness / Alembic Payload / Import Post-check / Plugin API Fixture | 检查 XGen/groom 到 Unreal 的 root UV、strand ID、guide curve、Alembic payload、Groom/Alembic API、目标 SkeletalMesh、cache receipt、import/post-check readiness 和 public plugin/API surface | R46 Maya L3 完成；R47 Unreal readiness L3 完成；R48 Maya `AbcExport` payload receipt 完成；R49 Unreal post-check readiness 完成，cache hash matched，AssetImportTask/Alembic factory 可 dry-run；R50 Groom Plugin/API Fixture Ready，Groom import API ready=true |

## 4. 当前开发进度

| 插件/工具线 | 完成度判断 | 能展示什么 | 不能展示什么 |
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、39 步 demo route、49 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
3. 每个 Maya 会话启动外部控制 bridge，然后外部 shell 控制：
   ...
   python <repo>\dcc-hosts\maya-auroraview-host\scripts\send_maya_command.py --export-presenter-pack r50-groom-plugin-api-fixture-presentation-pack
   ```
