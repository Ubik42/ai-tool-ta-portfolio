...
更新时间：2026-08-05  
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r43` / `dcc-first-package@1.40.0`
...
- Presenter Pack：`<repo>\dcc-hosts\maya-auroraview-host\artifacts\r43-unreal-control-rig-deformation-link-presentation-pack-20260805-233308.json`
...
- Presenter Pack 结果：41 / 41 evidence files present，0 missing required files，32 demo route steps。
...
| 角色 DNA、拓扑、joint coverage、面部/肌肉参数迁移 | `Character Calibration Studio` 已通过 Maya `mayapy` 采集 topology / joint / calibration / face params / Control Rig mapping facts；R35 Drilldown 已把 flat rows 转成 topology/skeleton/skin/calibration/face/Control Rig/mirror panels 和 owner actions；R42 已通过 Unreal Python 创建 public `CR_HeroFace`、写入 5 个 runtime controls，并复跑 Control Rig Bridge 让 approved 行 Ready；R43 继续只读检查 control -> deformation target -> Unreal Skeleton link 和 compile API surface，揭示 `Eye_L` / `Eye_R` / `Jaw` target 未匹配 | Maya L3 + L3-derived drilldown + Unreal Control Rig authoring L3 + post-authoring bridge L3 + deformation link L3 | direct compile status bridge、public face skeleton fixture、owner waiver |
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R43 Presenter Pack 41/41 evidence present；32 步 demo route；新增 Unreal Control Rig Deformation Link probe |
...
| 14 | Character Calibration & Intent Transfer Studio | 检查 DNA/拓扑/joint/面部参数/Unreal Control Rig 映射，避免“算法能跑但艺术表现错” | Maya `mayapy` L3 完成；R35 drilldown 完成；R42 Unreal Control Rig Fixture Authoring Ready；R43 Deformation Link L3 揭示 approved 行仍缺部分 Skeleton target match |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 96% | Maya 内打开工具、外部 command bridge、32 步 demo route、41 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Character Calibration Studio | 78% | Maya `mayapy` L3 采集 topology signature、joint coverage、calibration delta、face params、Control Rig mapping；R35 drilldown 输出 14 个 UI-ready panels、8 条 owner actions；R42 创建 public `CR_HeroFace`，写入 5 个 runtime controls；R43 只读链接 controls、Skeleton target 和 compile API visibility，揭示 deeper binding blocker | direct compile status bridge、public face skeleton fixture、owner waiver 还可深化 |
...
3. 每个 Maya 会话启动外部控制 bridge，然后外部 shell 控制：
   ...
   python <repo>\dcc-hosts\maya-auroraview-host\scripts\send_maya_command.py --export-presenter-pack r43-unreal-control-rig-deformation-link-presentation-pack
   ```
