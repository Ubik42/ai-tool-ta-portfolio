...
更新时间：2026-08-06
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r67` / `dcc-first-package@1.64.0`
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R67 Presenter Pack 65/65 evidence present；55 步 demo route；新增 Unreal Animation Attach Timing Readiness |
...
| 14 | Unreal Animation Bridge / Deep Facts / Attach Timing | 把 Maya 动画连续性 facts 映射到 Unreal AnimSequence/Skeleton/root motion/curve/compression runtime facts，并继续判断 gameplay attach timing 是否可发布 | Unreal import L3 完成；R41 deep facts 完成；R67 attach timing readiness 完成，notifyReadableIntents=0，missingAttachTimingEvents=2，正确 Blocked |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、55 步 demo route、65 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Unreal Animation Bridge | 68% | Maya 生成 FBX、Unreal Python 导入 Skeleton/SkeletalMesh/AnimSequence、2/2 sequences present；R41 只读采集 duration、derived frame span、frame-rate、root motion、compression metadata visibility；R67 把 gameplay attach intent 接到 AnimSequence notify/timing gate，missingAttachTimingEvents=2，assetWrites=0 | curve names / notify properties 在 UE Python 下不可读，后续需要 Animation Notify C++ / Editor Utility bridge |
