...
更新时间：2026-08-05  
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r48` / `dcc-first-package@1.45.0`
...
- Presenter Pack：`<repo>\dcc-hosts\maya-auroraview-host\artifacts\r48-groom-alembic-payload-presentation-pack-20260806-012304.json`
...
- Presenter Pack 结果：47 / 47 evidence files present，0 missing required files，37 demo route steps。
...
| 19 | Groom Export Inspector / Unreal Readiness / Alembic Payload | 检查 XGen/groom 到 Unreal 的 root UV、strand ID、guide curve、Alembic payload、Groom/Alembic API、目标 SkeletalMesh 和 cache receipt | R46 Maya L3 完成；R47 Unreal readiness L3 完成；R48 Maya `AbcExport` payload receipt 完成，approved groom 写出 `.abc`，TMP groom held |

## 4. 当前开发进度

| 插件/工具线 | 完成度判断 | 能展示什么 | 不能展示什么 |
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、37 步 demo route、47 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Groom Export Inspector | 72% | Maya `mayapy` L3 采集 root UV、strand ID、guide curve、Alembic payload、Unreal Groom/Binding intent；Unreal 5.3.2 L3 readiness 采集 AssetImportTask、AlembicImportFactory、target SkeletalMesh、Groom API 和期望 Groom/Binding 资产缺口；R48 Maya `AbcExport` 写出 approved public `.abc` cache 并记录 bytes/hash | 还没有 Unreal Alembic import/post-check 和 Groom plugin/API public fixture 复验 |
...
3. 每个 Maya 会话启动外部控制 bridge，然后外部 shell 控制：
   ...
   python <repo>\dcc-hosts\maya-auroraview-host\scripts\send_maya_command.py --export-presenter-pack r48-groom-alembic-payload-presentation-pack
   ```
