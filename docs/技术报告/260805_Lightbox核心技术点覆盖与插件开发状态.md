...
更新时间：2026-08-05  
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r49` / `dcc-first-package@1.46.0`
...
- Presenter Pack：`<repo>\dcc-hosts\maya-auroraview-host\artifacts\r49-groom-alembic-import-postcheck-presentation-pack-20260806-014423.json`
...
- Presenter Pack 结果：48 / 48 evidence files present，0 missing required files，38 demo route steps。
...
| Groom/XGen 到 Unreal | `Groom Export Inspector` 已通过 Maya `mayapy` 采集 synthetic scalp / curve strand facts，检查 root UV、strand ID、guide curve、Alembic payload 和 Unreal binding intent；R47 `Groom Unreal Import Readiness` 已通过 Unreal 5.3.2 采集 Groom/Alembic API visibility、target SkeletalMesh presence、expected Groom / Binding assets 和 zero-write boundary；R48 `Groom Alembic Payload Receipt` 已通过 Maya `AbcExport` 写出 approved public groom `.abc` cache 并记录 bytes/hash；R49 `Groom Alembic Import/Post-check Readiness` 已通过 Unreal 5.3.2 读取 `.abc`、验证 sha256 continuity、dry-run AssetImportTask、检查 Alembic factory / Groom API / target assets / no-write boundary | Maya L3 + Unreal readiness L3 + Maya Alembic cache L3 + Unreal post-check readiness L3 已完成 | Groom controlled executor、Groom plugin/API public fixture 复验 |

## 3. 计划中的插件线

| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R49 Presenter Pack 48/48 evidence present；38 步 demo route；新增 Groom Alembic import/post-check probe |
...
| 19 | Groom Export Inspector / Unreal Readiness / Alembic Payload / Import Post-check | 检查 XGen/groom 到 Unreal 的 root UV、strand ID、guide curve、Alembic payload、Groom/Alembic API、目标 SkeletalMesh、cache receipt 和 import/post-check readiness | R46 Maya L3 完成；R47 Unreal readiness L3 完成；R48 Maya `AbcExport` payload receipt 完成；R49 Unreal post-check readiness 完成，cache hash matched，AssetImportTask/Alembic factory 可 dry-run，Groom API 缺口被正确阻断 |

## 4. 当前开发进度

| 插件/工具线 | 完成度判断 | 能展示什么 | 不能展示什么 |
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、38 步 demo route、48 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Groom Export Inspector | 78% | Maya `mayapy` L3 采集 root UV、strand ID、guide curve、Alembic payload、Unreal Groom/Binding intent；Unreal 5.3.2 L3 readiness 采集 AssetImportTask、AlembicImportFactory、target SkeletalMesh、Groom API 和期望 Groom/Binding 资产缺口；R48 Maya `AbcExport` 写出 approved public `.abc` cache 并记录 bytes/hash；R49 Unreal 读取 `.abc` 并验证 sha256 continuity、AssetImportTask dry-run、Alembic factory visibility、target `SK_HeroFace` 和 no-write boundary | 还没有 Groom controlled executor、Groom plugin/API public fixture 复验 |
...
3. 每个 Maya 会话启动外部控制 bridge，然后外部 shell 控制：
   ...
   python <repo>\dcc-hosts\maya-auroraview-host\scripts\send_maya_command.py --export-presenter-pack r49-groom-alembic-import-postcheck-presentation-pack
   ```
