...
更新时间：2026-08-06
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r74` / `dcc-first-package@1.71.0`
...
- 3ds Max L3：`<repo>\dcc-hosts\3dsmax-rule-adapter\artifacts\max-rule-adapter-l3-20260806-032411.json`
- Presenter Pack 结果：72 / 72 evidence files present，0 missing required files，62 demo route steps。
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R74 Presenter Pack 72/72 evidence present；62 步 demo route；新增 Unreal Control Rig Native Bridge Readiness |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、61 步 demo route、71 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Character Calibration Studio | 86% | Maya `mayapy` L3 采集 topology signature、joint coverage、calibration delta、face params、Control Rig mapping；R35 drilldown 输出 14 个 UI-ready panels、8 条 owner actions；R42 创建 public `CR_HeroFace`，写入 5 个 runtime controls；R44 创建 public `SK_HeroFace_Skeleton` 并复跑 deformation-link；R45 调用 compile 方法并证明无 dirty/save 副作用；R74 新增 `AI_Tool_TA_ControlRigBridge` source/readiness，runtimeEntered=true，Control Rig / RigVM classes visible=true，missingRequiredNativeFiles=0，productionWrites=0 | native bridge BuildPlugin / commandlet probe、owner waiver 还可深化 |
...
下一轮不要再围绕 Blender/Max readiness、Groom StaticMesh importer、Groom group/root projection、socket Python authoring identity、R72 notify controlled write 接回 gameplay readiness 或 R74 Control Rig source readiness 打转，它们已进入真实 runtime 证据、明确 API-limited readiness 或 R73/R74 合流门禁。`3ds Max Rule Adapter` 已有 `pymxs` L3、R53 material slot -> texture manifest link 和 R58 controlled repair；`Houdini Rule Adapter` 已有 R56 HDA/PDG/bake receipt contract 和 hython readiness，只有在能定位 `hython.exe` 时才升级真实 L3；`Unreal Animation Bridge` 已有 import L3、R41 deep facts、R67 attach timing readiness、R68 native notify bridge source/readiness、R69/R72 native notify bridge build、R70 native notify commandlet probe、R71 native diagnostics、R72 native controlled write 和 R73 gameplay attach timing controlled readiness；`Character Calibration Studio` 已有 Maya L3、R35 drilldown、R42 Control Rig fixture authoring、post-authoring bridge、R43 deformation link、R44 face skeleton fixture、R45 compile status bridge 和 R74 native bridge readiness；`Groom Export Inspector` 已有 R46 Maya L3、R47 Unreal readiness L3、R52 curve-only Maya Alembic payload receipt、R52 Unreal import/post-check readiness、R50 plugin/API fixture、R52 controlled executor Ready rollback proof、R55 runtime fact collector 和 R59 group/root projection；`Spatial Authoring Workbench` 已有 Maya L3、R36 drilldown、R38 Unreal Socket Import Checker、R40 Socket Authoring Executor API-limited gate、R60-R73 native bridge source/build/commandlet/dry-run/controlled write/gameplay controlled readiness/animation timing readiness/native notify bridge readiness/build/runtime/diagnostics/controlled notify write/gameplay timing readiness；`Platform Variant Forge` 已完成 L3-linked plan、Unreal runtime-vs-plan L3、dry-run generation plan、texture runtime collector、public Texture2D payload fixture、controlled executor、executor receipts 和 StaticMesh post-check。后续优先做 Control Rig native bridge BuildPlugin / commandlet probe，或 MotionBuilder adapter。
