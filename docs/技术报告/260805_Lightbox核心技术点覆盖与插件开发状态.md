...
更新时间：2026-08-06
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r72` / `dcc-first-package@1.69.0`
...
- Presenter Pack：`<repo>\dcc-hosts\maya-auroraview-host\artifacts\r72-unreal-animation-notify-native-controlled-write-presentation-pack-20260806-091404.json`
...
- 3ds Max L3：`<repo>\dcc-hosts\3dsmax-rule-adapter\artifacts\max-rule-adapter-l3-20260806-032411.json`
- Presenter Pack 结果：70 / 70 evidence files present，0 missing required files，60 demo route steps。
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R72 Presenter Pack 70/70 evidence present；60 步 demo route；新增 Unreal Animation Notify Native Controlled Write |
...
| 14 | Unreal Animation Bridge / Deep Facts / Attach Timing / Native Notify Bridge | 把 Maya 动画连续性 facts 映射到 Unreal AnimSequence/Skeleton/root motion/curve/compression runtime facts，并继续判断 gameplay attach timing 是否可发布 | Unreal import L3 完成；R41 deep facts 完成；R67 attach timing readiness 完成；R68 native notify bridge source/readiness 完成；R69/R72 native notify bridge build 完成；R70 commandlet probe 完成；R71 native diagnostics 完成；R72 native controlled write 完成，2/2 notify applied/post-checked/rolled back，finalHashRestored=true |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、60 步 demo route、70 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
下一轮不要再围绕 Blender/Max readiness、Groom StaticMesh importer、Groom group/root projection 或 socket Python authoring identity 打转，它们已进入真实 runtime 证据或明确 API-limited readiness。`3ds Max Rule Adapter` 已有 `pymxs` L3、R53 material slot -> texture manifest link 和 R58 controlled repair；`Houdini Rule Adapter` 已有 R56 HDA/PDG/bake receipt contract 和 hython readiness，只有在能定位 `hython.exe` 时才升级真实 L3；`Unreal Animation Bridge` 已有 import L3、R41 deep facts、R67 attach timing readiness、R68 native notify bridge source/readiness、R69/R72 native notify bridge build、R70 native notify commandlet probe、R71 native diagnostics 和 R72 native controlled write；`Character Calibration Studio` 已有 Maya L3、R35 drilldown、R42 Control Rig fixture authoring、post-authoring bridge、R43 deformation link、R44 face skeleton fixture 和 R45 compile status bridge；`Groom Export Inspector` 已有 R46 Maya L3、R47 Unreal readiness L3、R52 curve-only Maya Alembic payload receipt、R52 Unreal import/post-check readiness、R50 plugin/API fixture、R52 controlled executor Ready rollback proof、R55 runtime fact collector 和 R59 group/root projection；`Spatial Authoring Workbench` 已有 Maya L3、R36 drilldown、R38 Unreal Socket Import Checker、R40 Socket Authoring Executor API-limited gate、R60-R72 native bridge source/build/commandlet/dry-run/controlled write/gameplay controlled readiness/animation timing readiness/native notify bridge readiness/build/runtime/diagnostics/controlled notify write；`Platform Variant Forge` 已完成 L3-linked plan、Unreal runtime-vs-plan L3、dry-run generation plan、texture runtime collector、public Texture2D payload fixture、controlled executor、executor receipts 和 StaticMesh post-check。后续优先把 R72 controlled write 接回 gameplay readiness；之后再做 MotionBuilder adapter 或 Control Rig C++ diagnostic bridge。
