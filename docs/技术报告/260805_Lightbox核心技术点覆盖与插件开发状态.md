...
更新时间：2026-08-05  
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r22` / `dcc-first-package@1.19.0`
...
- Presenter Pack：`<repo>\dcc-hosts\maya-auroraview-host\artifacts\r22-blender-max-l3-presentation-pack-20260805-153957.json`
...
- Presenter Pack 结果：19 / 19 evidence files present，0 missing required files，12 demo route steps。
- Gate 仍是 `CapturePending`，原因只剩 Maya GUI 截图/录屏未采集；Blender/Max 的 `Blocked` 是 synthetic fixture 中故意放入的失败资产，不是 runtime 缺失。
...
| 动画确定性导出、Take、sub-frame、channel identity | 已有方法提炼和 headless fixture | L2 | 开发 `Animation Continuity Lab`，跑 Maya/MotionBuilder/Unreal runtime |
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R22 Presenter Pack 19/19 evidence present；新增 command bridge |
...
| 12 | Animation Continuity Lab | 检查 Maya/MotionBuilder/Unreal 动画传递中的角色身份、Take、时间、通道和曲线差异 | 协议和 headless fixture 完成，插件未开发 |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 85% | Maya 内打开工具、外部 command bridge、12 步 demo route、19 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Animation Continuity Lab | 15% | 文件名归一、sub-frame、channel collision、MotionBuilder 状态机的协议验证 | 没有 DCC 插件 UI，没有 Maya/Mobu/Unreal runtime |
...
下一轮不要再围绕 Blender/Max readiness 打转，它们已升到 runtime L3。优先进入 `Animation Continuity Lab`：先做协议库、headless fixture、Maya animation fact collector，再接 MotionBuilder/Unreal 的 runtime 对照。它最能体现工具管线 TA 对“业务正确性”的理解，而不是只做资产静态检查。
