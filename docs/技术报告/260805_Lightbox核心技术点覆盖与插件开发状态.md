...
更新时间：2026-08-06
工程根目录：`<repo>`  
当前发布包：`ai-tool-ta-dcc-first-showcase-r69` / `dcc-first-package@1.66.0`
...
- 3ds Max L3：`<repo>\dcc-hosts\3dsmax-rule-adapter\artifacts\max-rule-adapter-l3-20260806-032411.json`
- Presenter Pack 结果：67 / 67 evidence files present，0 missing required files，57 demo route steps。
...
| # | 插件/工具线 | 大白话说明 | 当前进度 |
| --- | --- | --- | --- |
| 1 | Maya AuroraView Host / Presenter Pack | 在 Maya 里打开作品集工具，并把所有证据打包给 reviewer | 已可运行；R69 Presenter Pack 67/67 evidence present；57 步 demo route；新增 Unreal Animation Notify Native Bridge Build |
...
| 14 | Unreal Animation Bridge / Deep Facts / Attach Timing / Native Notify Bridge | 把 Maya 动画连续性 facts 映射到 Unreal AnimSequence/Skeleton/root motion/curve/compression runtime facts，并继续判断 gameplay attach timing 是否可发布 | Unreal import L3 完成；R41 deep facts 完成；R67 attach timing readiness 完成；R68 native notify bridge source/readiness 完成；R69 native notify bridge build 完成，compiledDlls=1，errorLines=0，commandlet diagnostics 待跑 |
...
| --- | --- | --- | --- |
| Maya Host / Presenter Pack | 98% | Maya 内打开工具、外部 command bridge、57 步 demo route、67 个证据文件探测 | 9 张截图和 1 段录屏未采集 |
...
| Unreal Animation Bridge | 78% | Maya 生成 FBX、Unreal Python 导入 Skeleton/SkeletalMesh/AnimSequence、2/2 sequences present；R41 只读采集 duration、derived frame span、frame-rate、root motion、compression metadata visibility；R67 把 gameplay attach intent 接到 AnimSequence notify/timing gate；R68 已新增 `AI_Tool_TA_AnimNotifyBridge` source/readiness，runtimeEntered=true，missingRequiredNativeFiles=0；R69 已通过 RunUAT BuildPlugin 编译 Win64 Editor DLL，compiledDlls=1，errorLines=0，sha256 已记录，productionWrites=0 | commandlet probe / native notify diagnostics 待跑；curve names / authored notify 写入仍待后续 |
