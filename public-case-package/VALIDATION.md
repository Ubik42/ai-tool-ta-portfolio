...
| --- | --- | --- |
| `validate-r51-groom-controlled-executor` | `python dcc-hosts/groom-export-inspector/scripts/run_groom_controlled_executor.py` | Unreal 5.3.2 Python selects the approved groom `.abc`, executes `AssetImportTask`, records the imported class as `StaticMesh` instead of `GroomAsset`, holds binding, rolls back public fixture writes, reports residual assets=0, and keeps engineWrites=0 / productionWrites=0. |
| `validate-r51-dcc-presenter-pack` | `Maya mayapy dcc_presentation_build_pack(label="r51-groom-controlled-executor-presentation-pack")` | DCC Presenter Pack probes 50 key evidence files including Groom Controlled Executor, reports 50 present / 0 missing required files, and exports 40 demo route steps. |
