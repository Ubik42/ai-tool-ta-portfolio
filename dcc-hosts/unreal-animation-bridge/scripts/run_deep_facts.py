...
SOURCE_ARTIFACT = ROOT / "artifacts" / "unreal-animation-bridge-import-l3-20260805-173309.json"
PROJECT_PATH = (
    PORTFOLIO_ROOT
    / "dcc-hosts"
...
UNREAL_SCRIPT = ROOT / "scripts" / "unreal_python" / "collect_animsequence_deep_facts.py"
...
def main() -> int:
    ...
    logs_dir = artifact_dir / "unreal-animation-deep-facts-logs"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    output_path = artifact_dir / ("unreal-animation-deep-facts-%s.json" % stamp)
    stdout_path = logs_dir / ("unreal-animation-deep-facts-%s.stdout.log" % stamp)
    stderr_path = logs_dir / ("unreal-animation-deep-facts-%s.stderr.log" % stamp)

    if not unreal_cli:
        return _blocked(output_path, "blocked_by_missing_unreal_cli", None)
    if not PROJECT_PATH.exists():
        return _blocked(output_path, "blocked_by_missing_unreal_project", unreal_cli)
    if not SOURCE_ARTIFACT.exists():
        return _blocked(output_path, "blocked_by_missing_unreal_animation_import_l3_artifact", unreal_cli)

    env = os.environ.copy()
    env["AI_TOOL_TA_UNREAL_ANIMATION_BRIDGE_ROOT"] = str(ROOT)
    env["AI_TOOL_TA_UNREAL_ANIMATION_DEEP_OUTPUT"] = str(output_path)
    env["AI_TOOL_TA_UNREAL_ANIMATION_DEEP_SOURCE"] = str(SOURCE_ARTIFACT)
...
def _blocked(output_path: Path, reason: str, unreal_cli: Optional[str]) -> int:
