...
LOG_DIR = ARTIFACT_DIR / "unreal-animation-native-notify-commandlet-logs"
RECEIPT_DIR = ARTIFACT_DIR / "unreal-animation-native-notify-commandlet-receipts"
...
TEMP_ROOT = Path(os.environ.get("AI_TOOL_TA_ANIM_NOTIFY_COMMANDLET_ROOT", r"D:\cs\_test\ai_tool_ta_anim_notify_commandlet_probe"))
REPORT_VERSION = "unreal-animation-notify-native-commandlet-probe@0.1.0"
...
def main() -> int:
    ...
    output_path = ARTIFACT_DIR / ("unreal-animation-notify-native-commandlet-probe-%s.json" % stamp)
    stdout_path = LOG_DIR / ("unreal-animation-notify-native-commandlet-probe-%s.stdout.log" % stamp)
    stderr_path = LOG_DIR / ("unreal-animation-notify-native-commandlet-probe-%s.stderr.log" % stamp)
    receipt_path = RECEIPT_DIR / ("unreal-animation-notify-native-commandlet-receipt-%s.json" % stamp)
    temp_project_dir = TEMP_ROOT / ("aanb-commandlet-%s" % stamp)
    temp_project = temp_project_dir / "AI_Tool_TA_AnimNotifyCommandletProbe.uproject"
...
def _resolve_build_artifact() -> Optional[Path]:
    ...
    if PUBLIC_MANIFEST.exists():
        ...
        path = _public_to_path(data.get("unrealAnimationNotifyNativeBridgeBuildArtifact"))
...
def _prepare_temp_project(temp_project_dir: Path, temp_project: Path, plugin_package: Path) -> None:
    ...
    project["Description"] = "Temporary AI Tool TA animation notify commandlet probe project."
...
def _report(
    ...
    return {
        ...
        "l3Status": "unreal_animation_notify_native_commandlet_loaded" if ready else (preflight_block or "unreal_animation_notify_native_commandlet_probe_blocked"),
        ...
        "summary": {
            ...
            "requestedAnimSequencePaths": output_payload.get("requestedAnimSequencePaths"),
        ...
        "reviewerClaims": [
            "The commandlet probe uses the R69 packaged AI_Tool_TA_AnimNotifyBridge plugin output, not a source-only contract.",
            "The Unreal project is copied to a temp folder under D:\\cs\\_test and the Anim Notify bridge plugin is enabled only there.",
            "The probe executes -run=AiToolTaAnimNotifyDiagnostics and treats the commandlet contract log plus readiness output JSON as runtime visibility proof.",
