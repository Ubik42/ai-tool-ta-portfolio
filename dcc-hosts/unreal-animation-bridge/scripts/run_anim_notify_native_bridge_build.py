...
PLUGIN = (
    ...
    / "Plugins"
    / "AI_Tool_TA_AnimNotifyBridge"
    / "AI_Tool_TA_AnimNotifyBridge.uplugin"
)
ARTIFACT_DIR = ROOT / "artifacts"
LOG_DIR = ARTIFACT_DIR / "unreal-animation-native-notify-build-logs"
BUILD_ROOT = Path(os.environ.get("AI_TOOL_TA_ANIM_NOTIFY_BRIDGE_BUILD_ROOT", r"D:\cs\_test\ai_tool_ta_anim_notify_builds"))
REPORT_VERSION = "unreal-animation-notify-native-bridge-build@0.1.0"
...
def main() -> int:
    ...
    output_path = ARTIFACT_DIR / ("unreal-animation-notify-native-bridge-build-%s.json" % stamp)
    stdout_path = LOG_DIR / ("unreal-animation-notify-native-bridge-build-%s.stdout.log" % stamp)
    stderr_path = LOG_DIR / ("unreal-animation-notify-native-bridge-build-%s.stderr.log" % stamp)
    package_dir = BUILD_ROOT / ("aanb-%s" % stamp)

    uat = _find_uat()
    toolchain = _toolchain_probe()
    preflight = _preflight(uat, toolchain)
    if preflight:
        report = _report(stamp, stdout_path, stderr_path, package_dir, uat, toolchain, preflight, None)
    ...
    report = _report(stamp, stdout_path, stderr_path, package_dir, uat, toolchain, None, build)
    output_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
...
def _preflight(uat: Optional[Path], toolchain: Dict[str, Any]) -> Optional[str]:
    ...
    if not PLUGIN.exists():
        return "blocked_by_missing_anim_notify_bridge_uplugin"
    required = [
        PLUGIN.parent / "Source" / "AI_Tool_TA_AnimNotifyBridge" / "AI_Tool_TA_AnimNotifyBridge.Build.cs",
        PLUGIN.parent / "Source" / "AI_Tool_TA_AnimNotifyBridge" / "Public" / "AiToolTaAnimNotifyDiagnosticsCommandlet.h",
        PLUGIN.parent / "Source" / "AI_Tool_TA_AnimNotifyBridge" / "Private" / "AiToolTaAnimNotifyDiagnosticsCommandlet.cpp",
        PLUGIN.parent / "Source" / "AI_Tool_TA_AnimNotifyBridge" / "Public" / "AiToolTaAnimNotifyBridgeLibrary.h",
        PLUGIN.parent / "Source" / "AI_Tool_TA_AnimNotifyBridge" / "Private" / "AiToolTaAnimNotifyBridgeLibrary.cpp",
    ]
...
def _report(
    ...
    return {
        ...
        "l3Status": "unreal_animation_notify_native_bridge_plugin_built" if succeeded else (preflight_block or "unreal_animation_notify_native_bridge_build_blocked"),
        ...
        "summary": {
            ...
            "stdoutLog": public_path(stdout_path),
            "stderrLog": public_path(stderr_path),
            "compilerVersion": (build.get("buildConfiguration") or {}).get("compilerVersion"),
            "configRestored": (build.get("buildConfiguration") or {}).get("restored"),
            "assetWrites": 0,
            "engineWrites": 0,
            "productionWrites": 0,
        },
        "reviewerClaims": [
            "The Anim Notify bridge build harness uses Unreal RunUAT BuildPlugin against the public Editor plugin source.",
            ...
            "No Unreal asset or production content is mutated by the build harness.",
        ],
    }
