...
SOURCE_ARTIFACT = PORTFOLIO_ROOT / "dcc-hosts" / "character-calibration-studio" / "artifacts" / "character-calibration-drilldown-20260805-202259.json"
UNREAL_PROJECT = (
    PORTFOLIO_ROOT
    / "dcc-hosts"
...
UNREAL_SCRIPT = ROOT / "scripts" / "unreal_python" / "probe_control_rig_bridge.py"
...
def main() -> int:
    ...
    logs_dir = artifact_dir / "unreal-control-rig-logs"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    output_path = artifact_dir / ("unreal-control-rig-bridge-l3-%s.json" % stamp)
    stdout_path = logs_dir / ("unreal-control-rig-bridge-%s.stdout.log" % stamp)
    stderr_path = logs_dir / ("unreal-control-rig-bridge-%s.stderr.log" % stamp)
    ...
    if not SOURCE_ARTIFACT.exists():
        return _blocked(output_path, "blocked_by_missing_character_drilldown_artifact", unreal_cli, str(UNREAL_PROJECT))

    env = os.environ.copy()
    env["AI_TOOL_TA_UNREAL_CONTROL_RIG_ROOT"] = str(ROOT)
    env["AI_TOOL_TA_UNREAL_CONTROL_RIG_OUTPUT"] = str(output_path)
    env["AI_TOOL_TA_UNREAL_CONTROL_RIG_SOURCE"] = str(SOURCE_ARTIFACT)
...
def _blocked(output_path: Path, reason: str, unreal_cli: str | None, project: str | None) -> int:
    ...
    from unreal_control_rig_bridge.contract import build_report
