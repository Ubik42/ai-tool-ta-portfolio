...
UNREAL_SCRIPT = ROOT / "scripts" / "unreal_python" / "collect_control_rig_deformation_link.py"
...
def main() -> int:
    ...
    output_path = artifact_dir / ("unreal-control-rig-deformation-link-%s.json" % stamp)
    stdout_path = logs_dir / ("unreal-control-rig-deformation-link-%s.stdout.log" % stamp)
    stderr_path = logs_dir / ("unreal-control-rig-deformation-link-%s.stderr.log" % stamp)
...
def _blocked(output_path: Path, reason: str, unreal_cli: Optional[str], project: Optional[str]) -> int:
    ...
    from unreal_control_rig_bridge.deformation_link import build_deformation_link_report
