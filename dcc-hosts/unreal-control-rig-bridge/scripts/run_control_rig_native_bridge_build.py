...
PLUGIN = (
    ...
    / "Plugins"
    / "AI_Tool_TA_ControlRigBridge"
    / "AI_Tool_TA_ControlRigBridge.uplugin"
)
ARTIFACT_DIR = ROOT / "artifacts"
LOG_DIR = ARTIFACT_DIR / "unreal-control-rig-native-bridge-build-logs"
BUILD_ROOT = Path(os.environ.get("AI_TOOL_TA_CONTROL_RIG_BRIDGE_BUILD_ROOT", r"D:\cs\_test\ai_tool_ta_control_rig_builds"))
REPORT_VERSION = "unreal-control-rig-native-bridge-build@0.1.0"
...
def main() -> int:
    ...
    output_path = ARTIFACT_DIR / ("unreal-control-rig-native-bridge-build-%s.json" % stamp)
    stdout_path = LOG_DIR / ("unreal-control-rig-native-bridge-build-%s.stdout.log" % stamp)
    stderr_path = LOG_DIR / ("unreal-control-rig-native-bridge-build-%s.stderr.log" % stamp)
    package_dir = BUILD_ROOT / ("acrnb-%s" % stamp)
        ...
        json.dumps(
            {
                ...
                "configRestored": summary["configRestored"],
            },
            ensure_ascii=False,
            indent=2,
        )
...
def _preflight(uat: Optional[Path], toolchain: Dict[str, Any]) -> Optional[str]:
    ...
    if not PLUGIN.exists():
        return "blocked_by_missing_control_rig_bridge_uplugin"
    required = [
        PLUGIN.parent / "Source" / "AI_Tool_TA_ControlRigBridge" / "AI_Tool_TA_ControlRigBridge.Build.cs",
        PLUGIN.parent / "Source" / "AI_Tool_TA_ControlRigBridge" / "Public" / "AiToolTaControlRigDiagnosticsCommandlet.h",
        PLUGIN.parent / "Source" / "AI_Tool_TA_ControlRigBridge" / "Private" / "AiToolTaControlRigDiagnosticsCommandlet.cpp",
        PLUGIN.parent / "Source" / "AI_Tool_TA_ControlRigBridge" / "Public" / "AiToolTaControlRigBridgeLibrary.h",
        PLUGIN.parent / "Source" / "AI_Tool_TA_ControlRigBridge" / "Private" / "AiToolTaControlRigBridgeLibrary.cpp",
    ]
...
def _report(
    ...
    return {
        ...
        "generatedBy": "AI Tool TA Portfolio / Unreal Control Rig Native Bridge Build",
        "generatedAt": time.strftime("%Y-%m-%d %H:%M:%S"),
        "evidenceLevel": "L3-build" if build else "Build-readiness",
        "l3Status": "unreal_control_rig_native_bridge_plugin_built" if succeeded else (preflight_block or "unreal_control_rig_native_bridge_build_blocked"),
        "sourcePlugin": public_path(PLUGIN),
        "artifact": public_path(output_path),
        ...
        "reviewerClaims": [
            "The Control Rig bridge build harness uses Unreal RunUAT BuildPlugin against the public Editor plugin source.",
            ...
            "No Unreal asset, engine file or production content is mutated by the build harness.",
        ],
    }
