...
SOURCE_DEFORMATION_ARTIFACT_FALLBACK = ROOT / "artifacts" / "unreal-control-rig-deformation-link-20260805-235154.json"
UNREAL_PROJECT = (
    PORTFOLIO_ROOT
    / "dcc-hosts"
...
UNREAL_SCRIPT = ROOT / "scripts" / "unreal_python" / "collect_control_rig_compile_status.py"
...
def resolve_source_deformation_artifact() -> Path:
    env_path = os.environ.get("AI_TOOL_TA_UNREAL_CONTROL_RIG_COMPILE_SOURCE")
    if env_path and Path(env_path).exists():
        return Path(env_path)
    candidates = sorted((ROOT / "artifacts").glob("unreal-control-rig-deformation-link-*.json"))
    return candidates[-1] if candidates else SOURCE_DEFORMATION_ARTIFACT_FALLBACK


def main() -> int:
    unreal_cli = find_unreal_cli()
    source_deformation_artifact = resolve_source_deformation_artifact()
    artifact_dir = ROOT / "artifacts"
    logs_dir = artifact_dir / "unreal-control-rig-compile-status-logs"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    output_path = artifact_dir / ("unreal-control-rig-compile-status-%s.json" % stamp)
    stdout_path = logs_dir / ("unreal-control-rig-compile-status-%s.stdout.log" % stamp)
    stderr_path = logs_dir / ("unreal-control-rig-compile-status-%s.stderr.log" % stamp)

    if not unreal_cli:
        return _blocked(output_path, "blocked_by_missing_unreal_cli", None, None, source_deformation_artifact)
    if not UNREAL_PROJECT.exists():
        return _blocked(output_path, "blocked_by_missing_unreal_project", unreal_cli, None, source_deformation_artifact)
    if not source_deformation_artifact.exists():
        return _blocked(output_path, "blocked_by_missing_deformation_link_artifact", unreal_cli, str(UNREAL_PROJECT), source_deformation_artifact)

    env = os.environ.copy()
    env["AI_TOOL_TA_UNREAL_CONTROL_RIG_ROOT"] = str(ROOT)
    env["AI_TOOL_TA_UNREAL_CONTROL_RIG_COMPILE_OUTPUT"] = str(output_path)
    env["AI_TOOL_TA_UNREAL_CONTROL_RIG_COMPILE_SOURCE"] = str(source_deformation_artifact)
    ...
    result = {
        ...
        "sourceDeformation": str(source_deformation_artifact),
    }
...
def _blocked(
    ...
    source_deformation_artifact: Path,
) -> int:
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    from unreal_control_rig_bridge.compile_status import build_compile_status_report
    ...
    report = build_compile_status_report(source_deformation_artifact, runtime_snapshot)
