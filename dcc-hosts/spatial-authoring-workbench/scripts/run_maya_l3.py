...
def main() -> int:
    ...
    try:
        from spatial_authoring_workbench.maya_collector import build_maya_report

        fixture = HOST_ROOT / "fixtures" / "synthetic_spatial_authoring_scene.json"
        artifact_dir = HOST_ROOT / "artifacts"
        artifact_dir.mkdir(parents=True, exist_ok=True)
        stamp = time.strftime("%Y%m%d-%H%M%S")
        output_path = artifact_dir / ("spatial-authoring-maya-l3-%s.json" % stamp)
