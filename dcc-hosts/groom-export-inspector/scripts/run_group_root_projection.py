...
SCRIPT = HOST_ROOT / "scripts" / "run_maya_group_root_projection.py"
...
def write_readiness_report(search: Dict[str, object]) -> Path:
    ...
    path = ARTIFACTS / ("groom-group-root-projection-readiness-%s.json" % time.strftime("%Y%m%d-%H%M%S"))
    report = {
        "reportVersion": "groom-group-root-projection-readiness@0.1.0",
        ...
        "collector": {
            ...
            "reportWhenAvailable": "groom-group-root-projection@0.1.0",
        },
        "boundary": {
...
def main() -> int:
    ...
    completed = subprocess.run([str(search["path"]), str(SCRIPT)], cwd=str(HOST_ROOT), text=True, timeout=300)
