...
from blender_rule_adapter.bpy_collector import build_bpy_report  # noqa: E402


def main() -> int:
    report = build_bpy_report(FIXTURE)
    ARTIFACTS.mkdir(parents=True, exist_ok=True)
    path = ARTIFACTS / ("blender-rule-adapter-l3-%s.json" % time.strftime("%Y%m%d-%H%M%S"))
