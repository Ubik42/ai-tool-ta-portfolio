...
from blender_rule_adapter.controlled_repair import build_controlled_repair_report  # noqa: E402


def main() -> int:
    report = build_controlled_repair_report(FIXTURE)
    ARTIFACTS.mkdir(parents=True, exist_ok=True)
    path = ARTIFACTS / ("blender-controlled-repair-%s.json" % time.strftime("%Y%m%d-%H%M%S"))
