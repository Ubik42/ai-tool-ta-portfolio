...
def main() -> int:
    ...
    try:
        from character_calibration_studio.maya_collector import build_maya_report

        fixture = HOST_ROOT / "fixtures" / "synthetic_character_calibration_scene.json"
        artifact_dir = HOST_ROOT / "artifacts"
        artifact_dir.mkdir(parents=True, exist_ok=True)
        stamp = time.strftime("%Y%m%d-%H%M%S")
        output_path = artifact_dir / ("character-calibration-maya-l3-%s.json" % stamp)
