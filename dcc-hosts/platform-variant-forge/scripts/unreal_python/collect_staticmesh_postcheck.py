...
def _main() -> None:
    ...
    output_path = Path(os.environ["AI_TOOL_TA_PLATFORM_VARIANT_STATICMESH_OUTPUT"])
    receipts_path = Path(os.environ["AI_TOOL_TA_PLATFORM_VARIANT_RECEIPTS"])
    if str(root) not in sys.path:
        sys.path.insert(0, str(root))

    import unreal  # type: ignore

    from platform_variant_forge.staticmesh_postcheck import build_staticmesh_postcheck_report

    receipts = json.loads(receipts_path.read_text(encoding="utf-8"))
    paths = _receipt_targets(receipts)
    runtime_snapshot = {
        "runtime": {
            "executed": True,
            ...
            "engineWrites": 0,
            "productionWrites": 0,
            "writeScope": "read-only /Game/AI_Tool_TA public fixture facts",
        },
        "facts": {path: _inspect_static_mesh(unreal, path) for path in paths},
        "apiAvailability": _api_availability(unreal),
    }
    report = build_staticmesh_postcheck_report(receipts_path, runtime_snapshot)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print("AI_TOOL_TA_PLATFORM_VARIANT_STATICMESH_OUTPUT=%s" % output_path)


def _receipt_targets(receipts: dict) -> list[str]:
    paths: set[str] = set()
    for receipt in receipts.get("receipts", []):
        params = receipt.get("deterministicParams", {})
        target = params.get("targetEnginePath")
        if target:
            paths.add(str(target))
        for write_path in receipt.get("rollbackReceipt", {}).get("writeSet", []):
            if write_path:
                paths.add(str(write_path))
    return sorted(paths)
...
def _inspect_static_mesh(unreal, asset_path: str) -> dict:
    ...
    collision = _collision_facts(static_mesh)
...
def _collision_facts(static_mesh) -> dict:
    if not static_mesh:
...
def _api_availability(unreal) -> dict:
    return {
        "EditorAssetLibrary": hasattr(unreal, "EditorAssetLibrary"),
        "EditorStaticMeshLibrary": hasattr(unreal, "EditorStaticMeshLibrary"),
        "StaticMesh": hasattr(unreal, "StaticMesh"),
        "StaticMeshEditorSubsystem": hasattr(unreal, "StaticMeshEditorSubsystem"),
    }
