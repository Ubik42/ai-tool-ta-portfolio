...
UNREAL_SCRIPT = ROOT / "scripts" / "unreal_python" / "probe_socket_import_checker.py"
...
def main() -> int:
    ...
    logs_dir = artifact_dir / "unreal-socket-logs"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    output_path = artifact_dir / ("unreal-socket-import-checker-l3-%s.json" % stamp)
    stdout_path = logs_dir / ("unreal-socket-import-checker-%s.stdout.log" % stamp)
    stderr_path = logs_dir / ("unreal-socket-import-checker-%s.stderr.log" % stamp)
    ...
    if not SOURCE_ARTIFACT.exists():
        return _blocked(output_path, "blocked_by_missing_spatial_drilldown_artifact", unreal_cli, str(UNREAL_PROJECT))

    env = os.environ.copy()
    env["AI_TOOL_TA_UNREAL_SOCKET_ROOT"] = str(ROOT)
    env["AI_TOOL_TA_UNREAL_SOCKET_OUTPUT"] = str(output_path)
    env["AI_TOOL_TA_UNREAL_SOCKET_SOURCE"] = str(SOURCE_ARTIFACT)
