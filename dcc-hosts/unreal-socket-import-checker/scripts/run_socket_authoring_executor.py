...
SOURCE_ARTIFACT = ROOT / "artifacts" / "unreal-socket-import-checker-l3-20260805-212131.json"
UNREAL_PROJECT = (
    PORTFOLIO_ROOT
    / "dcc-hosts"
...
UNREAL_SCRIPT = ROOT / "scripts" / "unreal_python" / "execute_socket_authoring.py"
...
def main() -> int:
    ...
    logs_dir = artifact_dir / "unreal-socket-authoring-executor-logs"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    output_path = artifact_dir / ("unreal-socket-authoring-executor-%s.json" % stamp)
    stdout_path = logs_dir / ("unreal-socket-authoring-executor-%s.stdout.log" % stamp)
    stderr_path = logs_dir / ("unreal-socket-authoring-executor-%s.stderr.log" % stamp)
    ...
    if not SOURCE_ARTIFACT.exists():
        return _blocked(output_path, "blocked_by_missing_unreal_socket_import_checker_artifact", unreal_cli, str(UNREAL_PROJECT))

    env = os.environ.copy()
    env["AI_TOOL_TA_UNREAL_SOCKET_ROOT"] = str(ROOT)
    env["AI_TOOL_TA_UNREAL_SOCKET_EXECUTOR_OUTPUT"] = str(output_path)
    env["AI_TOOL_TA_UNREAL_SOCKET_EXECUTOR_SOURCE"] = str(SOURCE_ARTIFACT)
        ...
        result.update(
            {
                ...
                "summary": report.get("executor", {}).get("summary"),
            }
        )
