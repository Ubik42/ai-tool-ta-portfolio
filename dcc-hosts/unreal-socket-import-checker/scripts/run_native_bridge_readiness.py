...
SOURCE_ARTIFACT = ROOT / "artifacts" / "unreal-socket-authoring-executor-20260805-222014.json"
UNREAL_PROJECT = (
    PORTFOLIO_ROOT
    / "dcc-hosts"
...
UNREAL_SCRIPT = ROOT / "scripts" / "unreal_python" / "probe_native_socket_bridge.py"
...
def main() -> int:
    ...
    logs_dir = artifact_dir / "unreal-socket-native-bridge-logs"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    output_path = artifact_dir / ("unreal-socket-native-bridge-readiness-%s.json" % stamp)
    stdout_path = logs_dir / ("unreal-socket-native-bridge-%s.stdout.log" % stamp)
    stderr_path = logs_dir / ("unreal-socket-native-bridge-%s.stderr.log" % stamp)
    ...
    if not SOURCE_ARTIFACT.exists():
        return _blocked(output_path, "blocked_by_missing_socket_authoring_executor_artifact", unreal_cli, str(UNREAL_PROJECT))

    env = os.environ.copy()
    env["AI_TOOL_TA_UNREAL_SOCKET_ROOT"] = str(ROOT)
    env["AI_TOOL_TA_UNREAL_SOCKET_NATIVE_BRIDGE_OUTPUT"] = str(output_path)
    env["AI_TOOL_TA_UNREAL_SOCKET_NATIVE_BRIDGE_SOURCE"] = str(SOURCE_ARTIFACT)
        ...
        result.update(
            {
                ...
                "sourceApiLimited": summary.get("sourceApiLimited"),
                "editorUtilitySurfaceVisible": summary.get("editorUtilitySurfaceVisible"),
                "socketClassesVisible": summary.get("socketClassesVisible"),
                "hasNativeSource": summary.get("hasNativeSource"),
                "hasSocketBridgePlugin": summary.get("hasSocketBridgePlugin"),
